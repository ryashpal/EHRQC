...
, labs_stg_2 as
	(
	  select
		...
		value_as_number,
		label,
		row_number() over (partition by person_id, label order by measurement_datetime __order__) as rn
	  from labs_stg_1
	)
, labs_stg_3 AS
(
...
(
    ...
    person_id,
    lactate AS lactate,
    carbondioxide_blood AS carbondioxide_blood,
    albumin AS albumin,
    glucose_urine AS glucose_urine,
    band_form_neutrophils AS band_form_neutrophils,
    base_excess_in_blood AS base_excess_in_blood,
    potassium_blood AS potassium_blood,
    ph_blood AS ph_blood,
    chloride_serum AS chloride_serum,
    carbondioxide_serum AS carbondioxide_serum,
    bilirubin AS bilirubin,
    leukocytes_blood_auto AS leukocytes_blood_auto,
    creatinine AS creatinine,
    inr AS inr,
    sodium_serum AS sodium_serum,
    sodium_blood AS sodium_blood,
    hemoglobin  AS hemoglobin,
    ph_bodyfluid AS ph_bodyfluid,
    platelet_count AS platelet_count,
    urea_nitrogen AS urea_nitrogen,
    glucose_serum AS glucose_serum,
    chloride_blood AS chloride_blood,
    oxygen AS oxygen,
    bicarbonate AS bicarbonate,
    potassium_serum AS potassium_serum,
    anion_gap AS anion_gap,
    leukocytes_blood_manual AS leukocytes_blood_manual,
    hematocrit AS hematocrit,
    aptt AS aptt
    FROM labs_stg_3
    ORDER BY person_id, rn asc NULLS LAST
)
