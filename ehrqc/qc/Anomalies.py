        ...
        with tag('body'):
            ...
            if missing:
                ...
                with tag('div'):
                    ...
                    drawMissingAnalysis(df, tag, text)
                doc.asis('<div style="clear:both;"></div>')
            if outliers:
...
def drawMissingAnalysis(df, tag, text):
    ...
    with tag('table table-dark', style='border: 1px solid black; border-collapse: collapse'):
        ...
        missingDf = df.isna().sum().sort_values()

        for (index, value) in missingDf.items():
            with tag('tr'):
                with tag('td', style='border: 1px solid black; border-collapse: collapse'):
                    text(index)
                missingProportion = round(value/df[index].count()*100, 2)
                if value > 0:
                    isMissing = True
                with tag('td', style='border: 1px solid black; border-collapse: collapse'):
                    text(str(value))
